...
export default class Bottombar {
    constructor(addFunc = () => { },
        ...
        const buttons = [];
        if (options.bottombar.canAdd) {
            buttons.push(new Icon('add').on('click', () => {
        ...
        buttons.push(h('span', '').child(this.moreEl));
        this.el = h('div', `${cssPrefix}-bottombar`).children(
            this.contextMenu.el,
            this.menuEl = h('ul', `${cssPrefix}-menu`).child(h('li', '').children(...buttons))
        );
        if(!options.bottombar.show){
            this.el.hide();
        }
    }

    addItem(name, options = {}, active) {
        this.dataNames.push(name);
        const item = h('li', active ? 'active' : '').child(name);
        item.on('click', () => {
            this.clickSwap2(item);
        }).on('contextmenu', (evt) => {
            if (options.bottombar.canDel) {
